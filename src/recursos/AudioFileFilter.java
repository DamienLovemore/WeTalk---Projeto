...
/**
 * Utilizada para definir um filtro que aceita apenas arquivos que sejam um tipo de arquivo de aúdio.
 * @author Matheus Soares Martins
 *
 */
public class AudioFileFilter extends FileFilter //O import correto aqui é javax.swing.filechooser.FileFilter e não java.io.FileFilter;
{
	//Define os formatos de arquivos aceitos
	private final String[] okFileExtensions={"mp3","wma","ape","flac","dts","m4a","aac","ac3","mmf","amr","m4r","ogg","wav","wv","mp2"};
	...
	public boolean accept(File file)
	{
		...
		{
			if(file.getName().toLowerCase().endsWith(extension)) //Pega o nome inteiro do arquivo incluido com a extensão (exemplo: mar.mp3), e converte isso para tudo minúsculo. Após isso vê se o nome do arquivo termina com uma das extensões válidas.
			{
				return true;
			}
		}
	...
	public String getDescription() //Pelo fato dele estar extendendo, ele deve implementar esse método obrigatório que retorna a descrição.
	{
		...
		retorno = "Audios"; //O nome do filtro que aparecerá na hora de selecionar os arquivos.
